...
namespace AntJoin.Core.Extensions
{
    ...
    public class EnumDescription : Attribute
    {
        ...
        /// <exception cref="NotSupportedException"></exception>
        /// <param name="enumType">枚举类型</param>
        /// <returns>所有定义的文本字典</returns>
        public static Dictionary<int?, EnumDescription> GetFieldTextDict(Type enumType)
        {
            Dictionary<int?, EnumDescription> descriptions = new Dictionary<int?, EnumDescription>();
            //缓存中没有找到，通过反射获得字段的描述信息
            if (cachedEnum.Contains(enumType.FullName) == false)
            {
                FieldInfo[] fields = enumType.GetFields();
            ...
            foreach (EnumDescription enumDes in (EnumDescription[])cachedEnum[enumType.FullName])
            {
                descriptions.Add(enumDes.EnumValue, enumDes);
            }
    ...
    public static class EnumExtensions
    {
        ...
        /// <returns></returns>
        public static string ToEnumDescription(this System.Enum @enum)
        {
            return EnumDescription.GetFieldText(@enum);
        }
    }
}
