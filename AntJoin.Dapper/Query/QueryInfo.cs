...
namespace AntJoin.Dapper
{
    ...
    public class QueryInfo : ICloneable
    {
                        ...
                        if (!string.IsNullOrEmpty(patten))//'LLK,GEQ'
                        {
                            ...
                            switch (patten)
                            {
                                ...
                                case "IN"://==> and a.Id in ('001','002')
                                    ...
                                    exp.AppendFormat("{0} in ('{1}')", sField, s.Replace(",", "','"));
                                    val = "$IN$";
                                    break;
        ...
        public QueryInfo AddParamIn<T>(string namedParam, IEnumerable<T> values)
        {
            Dictionary<string, object> dict = new Dictionary<string, object>
            {
                { namedParam + "_IN", string.Join(",", values) }
            };
            AddParam(dict);
            return this;
        }

        public QueryInfo AddParamIn(string namedParam, string sql)
        {
            this.Wheres.Add(namedParam + "_IN_SQL", " and " + namedParam + " in (" + sql + ")");
            return this;
        }
        public QueryInfo AddParamNotIn<T>(string namedParam, IEnumerable<T> values)
        {
            var sql = "'" + string.Join("','", values) + "'";
            this.Wheres.Add(namedParam + "_NOT_IN_SQL", " and " + namedParam + " not in (" + sql + ")");
            return this;
        }

        public QueryInfo AddParamNotIn(string namedParam, string sql)
        {
            this.Wheres.Add(namedParam + "_NOT_IN_SQL", " and " + namedParam + " not in (" + sql + ")");
            return this;
        }
        ...
        protected void AddParam(string alias, string colName, string namedParam, object value, string expression)
        {
            if (value == null || string.Empty.Equals(value))//by crabo 2016.09.26
                return;
            if (value.ToString() == "$IN$" || value.ToString() == "$NVL$")
                value = null;
        ...
        public object Clone()
        {
            ...
            info.HasNextPage = this.HasNextPage;
            return info;
        }
